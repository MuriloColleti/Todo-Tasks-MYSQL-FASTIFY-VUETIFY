...
export async function updateById(id_task, data) {
  return await database.query(
    'UPDATE tasks SET title = ?, body = ?, id_status = ? WHERE id_task = ?',
    [data.title, data.body, data.id_status, id_task],
  );
}
...
export async function create(data) {
  try {
    const result = await database.query(
      'INSERT INTO tasks (title, body, id_status) VALUES (?, ?, ?)',
      [data.title, data.body, data.id_status],
    );
...
export async function getTasksByUser(id_user) {
  const result = await database.query(
    `SELECT t1.*, t2.title AS status_title FROM tasks AS t1 
